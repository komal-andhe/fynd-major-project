...
// fetching data by id
    ...
    // todo:will not hard-code server url here
            const response =await fetch('http://localhost:4500/products/'+id);
...
export function fetchProductsByFilters({filter,pagination}){
    ...
    return new Promise(async(resolve)=>{
        ...
        const response =await fetch('http://localhost:4500/products?'+queryString);
        const data =await response.json();
...
export function fetchCategories(){
    return new Promise(async(resolve)=>{
        const response =await fetch('http://localhost:4500/categories');
        const data =await response.json();
        resolve({data});
